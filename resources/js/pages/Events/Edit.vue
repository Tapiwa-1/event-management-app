...
const props = defineProps<{ event: any; clients: any[] }>();
defineOptions({ layout: { breadcrumbs: [{ title: 'Events', href: index() }, { title: 'Edit', href: '#' }] } });
const form = useForm({ ...props.event });
const submit = () => form.patch(update.url(props.event.id));
</script>
...
<template>
    ...
    <div class="flex flex-1 flex-col gap-6 p-4">
        ...
        <form class="grid max-w-4xl gap-4 rounded-lg border bg-card p-4" @submit.prevent="submit">
            ...
            <button class="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground" :disabled="form.processing">Update Event</button>
        </form>
    </div>
</template>
