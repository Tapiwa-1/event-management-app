...
import { index, update } from '@/routes/packages';

const props = defineProps<{ packageTemplate: any; services: any[] }>();
defineOptions({ layout: { breadcrumbs: [{ title: 'Packages', href: index() }, { title: 'Edit package', href: '#' }] } });
const form = useForm({
    name: props.packageTemplate.name,
    event_type: props.packageTemplate.event_type,
    description: props.packageTemplate.description,
    is_active: props.packageTemplate.is_active,
    items: props.packageTemplate.items.map((item: any) => ({
...
const submit = () => form.patch(update.url(props.packageTemplate.id));
</script>

<template>
    <Head title="Edit Package" />
    <div class="flex flex-1 flex-col gap-6 p-4">
        <PageHeader title="Edit Saved Package" :description="`Package total: ${money(total)}`" />
        <form class="grid max-w-5xl gap-4 rounded-lg border bg-card p-4" @submit.prevent="submit">
            ...
            <div class="grid gap-3">
                <div v-for="(item, itemIndex) in form.items" :key="itemIndex" class="grid gap-3 rounded-md border p-3 md:grid-cols-[1fr_120px_160px]">
            ...
            <button class="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground" :disabled="form.processing">Update Package</button>
        </form>
    </div>
</template>
