...
import { index, store } from '@/routes/resources';
defineOptions({ layout: { breadcrumbs: [{ title: 'Resources', href: index() }, { title: 'New resource', href: '#' }] } });
const form = useForm({ name: '', type: 'PA System', quantity: 1, status: 'Available', notes: '' });
const submit = () => form.post(store.url());
</script>
<template>
    <Head title="New Resource" />
    <div class="flex flex-1 flex-col gap-6 p-4"><PageHeader title="New Resource" /><form class="grid max-w-3xl gap-4 rounded-lg border bg-card p-4" @submit.prevent="submit">
        ...
        <FormField label="Notes"><textarea v-model="form.notes" class="min-h-20 rounded-md border bg-background px-3 py-2" /></FormField><button class="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground">Save Resource</button>
    </form></div>
</template>
